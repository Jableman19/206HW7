...
def make_players_table(data, cur, conn):
    ...
    for player in data['squad']:
        ...
        pos = cur.execute("SELECT id FROM Positions WHERE position = ?", (player['position'],))
        player_positions.append(pos.fetchone()[0])
        birthYear = int(player['dateOfBirth'][:4])
        player_birthyears.append(birthYear)
    ...
    for i in range(len(player_ids)):
        # print(player_ids[i], player_names[i], player_positions[i], player_birthyears[i], player_nationalities[i]) 
        cur.execute("INSERT OR IGNORE INTO Players (id, name, position_id, birthyear, nationality) VALUES (?,?,?,?,?)",(player_ids[i] , str(player_names[i]), player_positions[i], player_birthyears[i], player_nationalities[i]))
    conn.commit()
...
def birthyear_nationality_search(age, country, cur, conn):
    preYear = 2023 - age
    tuples = []
    rows = cur.execute("SELECT name, nationality, birthyear FROM Players WHERE nationality = ? AND birthyear < ?", (country, preYear))
    for row in rows:
        tuples.append(row)
    return tuples
## [TASK 4]: 15 points
...
def position_birth_search(position, age, cur, conn):
    postYear = 2023 - age
    tuples = []
    rows = cur.execute("SELECT Players.name, Positions.position, Players.birthyear FROM Players JOIN Positions ON Players.position_id = Positions.id WHERE Positions.position = ? AND Players.birthyear > ?", (position, postYear))
    for row in rows:
        tuples.append(row)
    return tuples
...
def make_winners_table(data, cur, conn):
    ...
    for season in data["seasons"]:
        if season["winner"] != None:
            ...
            club_names.append(season["winner"]["name"])
    cur.execute("CREATE TABLE IF NOT EXISTS Winners (id INTEGER PRIMARY KEY, name TEXT)")
    for i in range(len(season_ids)):
        cur.execute("INSERT OR IGNORE INTO Winners (id, name) VALUES (?, ?)", (season_ids[i], club_names[i]))
    conn.commit()
...
def make_seasons_table(data, cur, conn):
    ...
    for season in data["seasons"]:
        if season["winner"] != None:
            season_ids.append(season["id"])
            endYear = season["endDate"][:4]
            end_years.append(int(endYear))
            winner_ids.append(season["winner"]["id"])
    cur.execute("CREATE TABLE IF NOT EXISTS Seasons (id INTEGER PRIMARY KEY, winner_id INTEGER, end_year INTEGER)")
    for i in range(len(season_ids)):
        cur.execute("INSERT OR IGNORE INTO Seasons (id, winner_id, end_year) VALUES (?, ?, ?)", (season_ids[i], winner_ids[i], end_years[i]))
    conn.commit()

def winners_since_search(year, cur, conn):
    year = int(year)
    cur.execute("SELECT name FROM Winners JOIN Seasons ON Winners.id = Seasons.id WHERE Seasons.end_year >= ?", (year,))
    row = cur.fetchone()
    winners = {}
    while row != None:
        # print(row)
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_winners_since_search(self):
        ...
        self.assertEqual(x['Manchester United FC'], 2)
